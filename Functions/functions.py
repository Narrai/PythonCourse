...
def center_text(*args, sep='', end='\n', file=None, flush=False):
    # text = str(text)  # cast text into string
    text = ""
    for arg in args:
        text += str(arg) + sep
...
# out put the file in the text file
with open("centered", 'w') as centered_file:
    center_text("spam and text", file=centered_file)
    center_text("spam, spam and eggs", file=centered_file)
    center_text(12, file=centered_file)  # Throws error and it is true binary(1100), hexadecimal(C)
